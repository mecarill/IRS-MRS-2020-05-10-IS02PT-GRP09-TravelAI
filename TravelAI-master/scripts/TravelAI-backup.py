...
if __name__ == '__main__':
    client_id = 'TbDAbPEIGXeA0RuFtcCr4fQGGz1ZjGvE' #sys.argv[1]
    client_secret = 'XrXTr1lIqUuvccQr'   #sys.argv[2]
    #cities = ['singapore','taipei','bangkok'] #sys.argv[3]
    from_date = '2020-08-01'    #sys.argv[4]
    to_date = '2020-08-03'  #sys.argv[5]
    #hotels = ['fullerton','fullerton']    #sys.argv[6]
    ratings = ['4','5']#sys.argv[7]
    ...
    #to_date = sys.argv[5]
    hotels = json.loads(sys.argv[6])
    #ratings = json.loads(sys.argv[7])
    #age = sys.argv[8]
    sex = sys.argv[9]
    travel_type = sys.argv[10]
    ...
    # # convert cities, location, and ratings into list
    #cities = [x for x in cities.split(',')]
    #hotels = [x for x in hotels.split(',')]
    #ratings = [x for x in ratings.split(',')]
    ...
    # data pulling
    try:
        flightdata = FlightData(cities_code, from_date, to_date, client_id, client_secret, base_url='https://api.amadeus.com')
        flightdata=flightdata.save_all_flights_in_df()
    except Exception as e:
        message['error']=message['error']+'Flightdata error: ' + str(e) +'     | '

    try:
        hoteldata = HotelData(cities_code[1:], hotels, ratings, from_date, to_date, client_id, client_secret, base_url='https://api.amadeus.com')
    ...
    # optimiser
    try:
        optimiseddata=Optimiser(flightdata,hoteldata)
    except Exception as e:
        message['error'] = message['error'] + 'Optimiser error: ' + str(e) +'     | '
        optimiseddata='[]'

    errordata=json.dumps(message)
    jsonoutput='[' + POI_json+',' + optimiseddata+ ',' + errordata+ ']'
    print(jsonoutput)
